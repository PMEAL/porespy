...
class ToolsTest():
    ...
    def test_numba_insert_disk_2D(self):
        ...
        im = ps.tools.insert_disk_at_points(im=im, coords=c, r=10, v=2)
        assert im.max() == 2
        assert im.sum() == 1220
        im = ps.tools.insert_disk_at_points(im=im, coords=c, r=10, v=2.4,
                                            smooth=False)
        assert im.max() == 2
        assert im.sum() == 1266
        im = ps.tools.insert_disk_at_points(im=im, coords=c, r=10, v=3,
                                            overwrite=False)
        assert im.max() == 2
        im = ps.tools.insert_disk_at_points(im=im, coords=c, r=10, v=3,
                                            overwrite=True)
    ...
    def test_numba_insert_disks_2D(self):
        ...
        im = ps.tools.insert_disks_at_points(im=im, coords=c, radii=r, v=2)
        assert im.max() == 2
        assert im.sum() == 1220
        im = ps.tools.insert_disks_at_points(im=im, coords=c, radii=r, v=2.4,
                                             smooth=False)
        assert im.max() == 2
        assert im.sum() == 1266
        im = ps.tools.insert_disks_at_points(im=im, coords=c, radii=r, v=3,
                                             overwrite=False)
        assert im.max() == 2
        im = ps.tools.insert_disks_at_points(im=im, coords=c, radii=r, v=3,
                                             overwrite=True)
