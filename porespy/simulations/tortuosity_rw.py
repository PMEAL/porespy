...
def tortuosity_rw(im,
    ...
    simulation and post-processing steps in one go. If more customization is desired,
    the functions are available separately as well.
        ...
        -------
        walk : Results
            Returns a Results object containing as named attributes:
            ...
            eDiff:
                A 1D numpy array of the effective diffusivity in all directions at each step
            eDiff_x:
                A 1D numpy array of the effective diffusivity in x-direction at each step
            eDiff_y:
                 A 1D numpy array of the effective diffusivity in y-direction at each step
            eDiff_z:
                 A 1D numpy array of the effective diffusivity in z-direction at each step
            displacement: ndarray
                A 2D array of the magnitude of each walker displacement at each step
            displacement_x : ndarray
                A 2D array of the displacements of each walker at each step in x-direction
            displacement_y : ndarray
                A 2D array of the displacements of each walker at each step in y-direction
            displacement_z : ndarray
                A 2D array of the displacements of each walker at each step in z-direction
            step_size : float
    ...
    if walk.porosity > 0.90:
        ...
        ax[0].plot([0, s[-1]], [walk.Db, walk.Db], '-', label='Theoretical Bulk Diffusion in Free Space')
...
if __name__ == '__main__':
    ...
    stride = 20
    seed = 2357
    B = tortuosity_rw(im, resolution=5, n_walkers=n_walkers, chunks=20, n_steps=n_steps, seed=seed, stride=stride, mode='axial')
