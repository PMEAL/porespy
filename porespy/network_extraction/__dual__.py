...
def dual(solid_regions, pore_regions, im, pore_dt=None, solid_dt=None, voxel_size=1):
    ...
    #            continue
        ...
        ext_pore_on_solid_slice=extend_slice(pore_on_solid_slice[pore], p_im.shape)
        sub_im = p_im[ext_p_slice]  # Sub image of pore extended slice
        sub_dt = p_dt[ext_p_slice]  # Sub distance transform of pore extended slice
        ...
        solid_labels_on_pore=(pore_region_mask*pore_regions_full!=0)*solid_regions_full
        neigbhour_solid_labels=sp.unique(solid_labels_on_pore)
        neigbhour_solid_labels=sp.delete(neigbhour_solid_labels,sp.where(neigbhour_solid_labels==0))
        ...
        #%% Old Code of porespy
        pore_im = sub_im == i   #  True/False form Sub image of ith pore extended slice
    ...
    for k in tqdm(Ss):
        ...
        ext_solid_on_pore_slice=extend_slice(solid_on_pore_slice[solid], s_im.shape)
        sub_im_s = s_im[ext_s_slice]  # Sub image of solid extended slice
        sub_dt_s = s_dt[ext_s_slice]  # Sub distance transform of solid extended slice
        ...
        pore_labels_on_solid=(solid_region_mask*solid_region_full!=0)*pore_region_full
        neigbhour_pore_labels=sp.unique(pore_labels_on_solid)
        neigbhour_pore_labels=sp.delete(neigbhour_pore_labels,sp.where(neigbhour_pore_labels==0))
        ...
        im_w_s_throats = spim.binary_dilation(input=im_solid_s, structure=disk(1))
        ...
        for l in Sn:
            if l > solid:
                ...
                temp_s = sp.where(s_dt[st_inds] == sp.amax(s_dt[st_inds]))[0][0]
                if s_im.ndim == 2:
    ...
    net['pore.solid_surface_area'] = sp.copy(bond_surf_area_phase)*(voxel_size)**2
    ...
    net['throat.equivalent_diameter'] = (sp.array(site_area)*(voxel_size**2))**(0.5)
    net['throat.conns'] = sp.array(site_conns_partial)
    P12 = net['throat.conns']
    PT1 = sp.sqrt(sp.sum(((bond_coords[P12[:, 0]]-site_coords)*voxel_size)**2, axis=1))
    PT2 = sp.sqrt(sp.sum(((bond_coords[P12[:, 1]]-site_coords)*voxel_size)**2, axis=1))
