...
def unpad(im, pad_width):
    r"""
    Remove padding from an image then extracts the result corresponding to
    ...
    if isinstance(pad_width, list) and np.ndim(pad_width) == 1:
    ...
    if isinstance(pad_width, list) and np.ndim(pad_width) == 2:
        pad_width = np.asarray(pad_width)
